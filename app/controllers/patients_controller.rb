class PatientsController < ApplicationController
  ...
  def show
    ...
    @p_schedules = @patient_schedules.map do |p_schedule| 
      {
        id: p_schedule.id,
        title: "#{p_schedule.patient_type.name}",
        start: p_schedule.start_at,
        end: p_schedule.end_at,
        color: p_schedule.patient_type.color,  # アクティブハッシュからカラーを取得
        className: 'patient'  # 患者用のクラスを追加
      }    
    end
    ...
    @d_schedules = @doctor_schedules.map do |d_schedule|
      {
        id: d_schedule.id,
        title: "#{d_schedule.doctor_type.name}",
        start: d_schedule.start_at,
        end: d_schedule.end_at,
        color: d_schedule.doctor_type.color, # カラーを取得
        className: 'doctor'  # 医者用のクラスを追加
      }    
    end
